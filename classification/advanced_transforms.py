...
class RandomBlur:
    """Apply random blur."""
    def __init__(self, p=0.1):  # Giảm từ 0.5 xuống 0.1
        self.p = p
    ...
    def __call__(self, img):
        ...
        if blur_type == 'gaussian':
            radius = random.uniform(0.5, 1.0)  # Giảm radius
            img = img.filter(ImageFilter.GaussianBlur(radius))
        elif blur_type == 'motion':
            kernel_size = random.choice([3, 5])  # Giảm kernel size
...
class RandomNoise:
    """Add random noise."""
    def __init__(self, p=0.1, noise_type='all'):  # Giảm từ 0.5 xuống 0.1
        self.p = p
    ...
    def __call__(self, img):
        ...
        if noise_type == 'gaussian':
            noise = np.random.normal(0, 0.01, img_array.shape)  # Giảm nhiễu
            img_array = img_array + noise
        elif noise_type == 'salt_pepper':
            prob = 0.01  # Giảm xác suất
...
class AdvancedCutout:
    ...
    def __init__(self, n_holes=1, length=20, strategy='random'):  # Giảm n_holes và length
