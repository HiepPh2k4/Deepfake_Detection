...
if __name__ == '__main__':
    ...
    MODEL_PATH = "G:/Hiep/Deepfake_Detection/classification/models/model_xception"
    OUTPUT_PATH = "G:/Hiep/Deepfake_Detection/classification/output/output_xception"
    BATCH_SIZE = 32
    NUM_WORKERS = 8
    LEARNING_RATE = 2e-4
    WEIGHT_DECAY = 1e-4
    ...
    # Prepare Xception model
    model = xception(num_classes=1, pretrained='imagenet')
    model.last_linear = nn.Linear(model.last_linear.in_features, 1)  # Adjust for binary classification
    ...
    model, metrics, train_losses, val_losses, train_accs, val_accs = train_and_evaluate(
        ...
        model_name="Xception", num_epochs=PRETRAIN_EPOCHS, device=DEVICE, save_dir=OUTPUT_PATH, patience=PATIENCE
    )
    ...
    model, metrics, train_losses, val_losses, train_accs, val_accs = train_and_evaluate(
        model, train_loader, val_loader, test_loader, criterion, optimizer,
        model_name="Xception", num_epochs=FINETUNE_EPOCHS, device=DEVICE, save_dir=OUTPUT_PATH, patience=PATIENCE
    )

    # Save final model
    os.makedirs(MODEL_PATH, exist_ok=True)
    torch.save({'model_state_dict': model.state_dict()}, os.path.join(MODEL_PATH, "xception_deepfake.pth"))
    print("Saved model Xception!")