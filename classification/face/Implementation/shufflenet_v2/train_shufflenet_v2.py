...
if __name__ == '__main__':
    ...
    model, metrics, train_losses, val_losses, train_accs, val_accs = train_and_evaluate(
        ...
        model_name="ShuffleNetV2", num_epochs=PRETRAIN_EPOCHS, device=DEVICE, save_dir=OUTPUT_PATH, patience=PATIENCE
    )
    ...
    model, metrics, train_losses, val_losses, train_accs, val_accs = train_and_evaluate(
        model, train_loader, val_loader, test_loader, criterion, optimizer,
        model_name="ShuffleNetV2", num_epochs=FINETUNE_EPOCHS, device=DEVICE, save_dir=OUTPUT_PATH, patience=PATIENCE
    )

    # Save final model
    os.makedirs(MODEL_PATH, exist_ok=True)
    torch.save({'model_state_dict': model.state_dict()}, os.path.join(MODEL_PATH, "shufflenet_v2_deepfake.pth"))
