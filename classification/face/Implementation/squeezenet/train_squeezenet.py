...
if __name__ == '__main__':
    ...
    MODEL_PATH = "G:/Hiep/Deepfake_Detection/classification/models/model_squeezenet"
    OUTPUT_PATH = "G:/Hiep/Deepfake_Detection/classification/output/output_squeezenet"
    BATCH_SIZE = 32
    NUM_WORKERS = 8
    LEARNING_RATE = 2e-4
    WEIGHT_DECAY = 1e-4
    ...
    model, metrics, train_losses, val_losses, train_accs, val_accs = train_and_evaluate(
        ...
        model_name="SqueezeNet", num_epochs=PRETRAIN_EPOCHS, device=DEVICE, save_dir=OUTPUT_PATH, patience=PATIENCE
    )
    ...
    model, metrics, train_losses, val_losses, train_accs, val_accs = train_and_evaluate(
        model, train_loader, val_loader, test_loader, criterion, optimizer,
        model_name="SqueezeNet", num_epochs=FINETUNE_EPOCHS, device=DEVICE, save_dir=OUTPUT_PATH, patience=PATIENCE
    )

    # Save final model
    os.makedirs(MODEL_PATH, exist_ok=True)
    torch.save({'model_state_dict': model.state_dict()}, os.path.join(MODEL_PATH, "squeezenet_deepfake.pth"))
    print("Saved model SqueezeNet!")