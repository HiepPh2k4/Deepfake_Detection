...
from classification.face.Implementation.xception_optimized.advanced_xception import OptimizedXceptionNet  # Import mô hình từ advanced_xception.py
...
# Tải mô hình
model = OptimizedXceptionNet(num_classes=1, dropout_rate=0.5).to(device)
checkpoint = torch.load('G:/Hiep/Deepfake_Detection/classification/models/model_remote_test_2/best_model.pth', map_location=device)
model.load_state_dict(checkpoint['model_state_dict'])  # Truy cập model_state_dict
model.eval()
print("Đã tải mô hình OptimizedXceptionNet!")
