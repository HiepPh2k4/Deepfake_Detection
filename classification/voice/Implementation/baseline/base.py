...
MODEL_CONFIGS = {
    'squeezenet': {'lr_multiplier': 0.01, 'batch_size': 24},  # Reduced for stability
...
class BaselineTrainer:
    ...
    def _train_epoch(self):
        ...
        for images, labels in tqdm(self.train_loader, desc=f"Epoch"):
            ...
            if torch.isnan(images).any():
                print("NaN detected in input images during training")
                continue
            ...
            torch.nn.utils.clip_grad_norm_(self.model.parameters(), max_norm=0.5)  # Reduced for stability
    ...
    def train(self):
        ...
        train_f1s, val_f1s = [], []
        train_aucs, val_aucs = [], []
        best_val_loss = float('inf')  # Track validation loss best
        ...
        for epoch in range(NUM_EPOCHS):
            ...
            # Save best model if val_loss lower
    ...
    def _validate(self):
        ...
        with torch.no_grad():
            for images, labels in self.val_loader:
                images, labels = images.to(DEVICE), labels.to(DEVICE)
                if torch.isnan(images).any():
                    print("NaN detected in input images during validation")
                    continue
                with autocast('cuda'):
                    outputs = self.model(images)
                    if torch.isnan(outputs).any():
                        print("NaN detected in model outputs during validation")
                        continue
                    loss = self.criterion(outputs, labels)
                total_loss += loss.item()
    ...
    def evaluate(self):
        ...
        with torch.no_grad():
            for batch_idx, (images, labels) in enumerate(self.test_loader):
                images, labels = images.to(DEVICE), labels.to(DEVICE)
                if torch.isnan(images).any():
                    print(f"NaN detected in input images at batch {batch_idx}")
                    continue
                with autocast('cuda'):
                    outputs = self.model(images)
                    if torch.isnan(outputs).any():
